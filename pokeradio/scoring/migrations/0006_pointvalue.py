...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Point = orm['scoring.Point']
        Point.objects.filter(action='TRACK_LIKED').update(value=1)
        Point.objects.filter(action='TRACK_DISLIKED').update(value=-1)

    def backwards(self, orm):
        "Write your backwards methods here."
        Point = orm['scoring.Point']
        Point.objects.filter(action='TRACK_LIKED').update(value=5)
        Point.objects.filter(action='TRACK_DISLIKED').update(value=-2)
